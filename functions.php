...
function employee_full_name($first_name, $middle_name, $last_name) {
    $name_parts = array_filter(array(
        trim((string)$first_name),
        trim((string)$middle_name),
...
function employee_split_full_name($full_name) {
    $name_parts = preg_split('/\s+/', trim((string)$full_name));

    if (empty($name_parts) || $name_parts[0] === '') {
        return array('', '', '');
    }

    if (count($name_parts) === 1) {
        return array($name_parts[0], '', '');
    }
...
function employee_name_from_row($row) {
